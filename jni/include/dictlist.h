...
class DictList {
 private:
  ...
  // char16
  IME_SIZE_T start_pos_[kMaxLemmaSize + 1];

  IME_SIZE_T start_id_[kMaxLemmaSize + 1];

  int (*cmp_func_[kMaxLemmaSize])(const void *, const void *);

  bool alloc_resource(IME_SIZE_T buf_size, IME_SIZE_T scim_num);

  void free_resource();

#ifdef ___BUILD_MODEL___
  // Calculate the requsted memory, including the start_pos[] buffer.
  IME_SIZE_T calculate_size(const LemmaEntry *lemma_arr, IME_SIZE_T lemma_num);

  void fill_scis(const SingleCharItem *scis, IME_SIZE_T scis_num);

  // Copy the related content to the inner buffer
  // It should be called after calculate_size()
  void fill_list(const LemmaEntry *lemma_arr, IME_SIZE_T lemma_num);
 ...
 public:
  ...
  // ids from 1
  bool init_list(const SingleCharItem *scis, IME_SIZE_T scis_num,
