...
class UserDict : public AtomDictBase {
 public:
  ...
  IME_SIZE_T get_total_lemma_count();
...
#define COARSE_UTC(year, month, day, hour, minute, second) \
...
#ifdef ___SYNC_ENABLED___
  uint32 * syncs_;
  IME_SIZE_T sync_count_size_;
#endif
  uint32 * offsets_by_id_;

  IME_SIZE_T lemma_count_left_;
  IME_SIZE_T lemma_size_left_;
  ...
  IME_SIZE_T _get_lpis(const uint16 *splid_str, uint16 splid_str_len,
                   LmaPsbItem *lpi_items, IME_SIZE_T lpi_max, bool * need_extend);
