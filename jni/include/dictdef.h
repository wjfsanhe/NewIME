...
const IME_SIZE_T kMaxSpellingNum = 512 - kHalfSpellingIdNum - 1;
const IME_SIZE_T kMaxSearchSteps = 40;

// One character predicts its following characters.
const IME_SIZE_T kMaxPredictSize = (kMaxLemmaSize - 1);

// LemmaIdType must always be IME_SIZE_T.
typedef IME_SIZE_T LemmaIdType;
const IME_SIZE_T kLemmaIdSize = 3;  // Actually, a Id occupies 3 bytes in storage.
const IME_SIZE_T kLemmaIdComposing = 0xffffff;

typedef uint16 LmaScoreType;
typedef uint16 KeyScoreType;

// Number of items with highest score are kept for prediction purpose.
const IME_SIZE_T kTopScoreLemmaNum = 10;

const IME_SIZE_T kMaxPredictNumByGt3 = 1;
const IME_SIZE_T kMaxPredictNumBy3 = 2;
const IME_SIZE_T kMaxPredictNumBy2 = 2;
 ...
 */
struct LmaNodeLE0 {
  IME_SIZE_T son_1st_off;
  IME_SIZE_T homo_idx_buf_off;
  uint16 spl_idx;
  uint16 num_of_son;
  uint16 num_of_homo;
};
