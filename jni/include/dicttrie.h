...
class DictTrie : AtomDictBase {
 private:
  typedef struct ParsingMark {
    IME_SIZE_T node_offset:24;
    IME_SIZE_T node_num:8;           // Number of nodes with this spelling id given
  ...
  IME_SIZE_T lma_node_num_le0_;
  IME_SIZE_T lma_node_num_ge1_;

  // The first part is for homophnies, and the last  top_lma_num_ items are
  // lemmas with highest scores.
  unsigned char *lma_idx_buf_;
  IME_SIZE_T lma_idx_buf_len_;  // The total size of lma_idx_buf_ in byte.
  IME_SIZE_T total_lma_num_;    // Total number of lemmas in this dictionary.
  IME_SIZE_T top_lmas_num_;     // Number of lemma with highest scores.
  ...
  inline IME_SIZE_T get_son_offset(const LmaNodeGE1 *node);

  // Get the offset of homonious ids for a node.
  inline IME_SIZE_T get_homo_idx_buf_offset(const LmaNodeGE1 *node);

  // Get the lemma id by the offset.
  inline LemmaIdType get_lemma_id(IME_SIZE_T id_offset);
  ...
  IME_SIZE_T fill_lpi_buffer(LmaPsbItem lpi_items[], IME_SIZE_T max_size,
                         LmaNodeLE0 *node);
 ...
 public:
  ...
  IME_SIZE_T number_of_lemmas() {return 0;}
  ...
  IME_SIZE_T get_lpis(const uint16 *splid_str, uint16 splid_str_len,
  ...
  IME_SIZE_T predict(const char16 *last_hzs, uint16 hzs_len,
  ...
  IME_SIZE_T get_total_lemma_count() {return 0;}
  void set_total_lemma_count_of_others(IME_SIZE_T count);
  ...
  IME_SIZE_T predict_top_lmas(IME_SIZE_T his_len, NPredictItem *npre_items,
