...
const IME_SIZE_T kMaxSpellingSize = kMaxPinyinSize;
...
class SpellingTable {
 private:
  static const IME_SIZE_T kNotSupportNum = 3;
  static const char kNotSupportList[kNotSupportNum][kMaxSpellingSize + 1];

  bool need_score_;

  IME_SIZE_T spelling_max_num_;
  ...
  IME_SIZE_T get_hash_pos(const char* spelling_str);
  IME_SIZE_T hash_pos_next(IME_SIZE_T hash_pos);
  void free_resource();
 public:
  SpellingTable();
  ~SpellingTable();
  ...
  bool init_table(IME_SIZE_T pure_spl_size, IME_SIZE_T spl_max_num, bool need_score);
  ...
  const char* arrange(IME_SIZE_T *item_size, IME_SIZE_T *spl_num);
