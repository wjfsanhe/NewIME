...
class DictBuilder {
 private:
  ...
  IME_SIZE_T lma_nds_used_num_le0_;
  IME_SIZE_T lma_nds_used_num_ge1_;
  ...
  IME_SIZE_T homo_idx_num_eq1_;
  // Number of homophonies each of which contains more than one character.
  IME_SIZE_T homo_idx_num_gt1_;

  // The items with highest scores.
  LemmaEntry *top_lmas_;
  IME_SIZE_T top_lmas_num_;

  SpellingTable *spl_table_;
  SpellingParser *spl_parser_;

#ifdef ___DO_STATISTICS___
  IME_SIZE_T max_sonbuf_len_[kMaxLemmaSize];
  IME_SIZE_T max_homobuf_len_[kMaxLemmaSize];

  IME_SIZE_T total_son_num_[kMaxLemmaSize];
  IME_SIZE_T total_node_hasson_[kMaxLemmaSize];
  IME_SIZE_T total_sonbuf_num_[kMaxLemmaSize];
  IME_SIZE_T total_sonbuf_allnoson_[kMaxLemmaSize];
  IME_SIZE_T total_node_in_sonbuf_allnoson_[kMaxLemmaSize];
  IME_SIZE_T total_homo_num_[kMaxLemmaSize];

  IME_SIZE_T sonbufs_num1_;     // Number of son buffer with only 1 son
  IME_SIZE_T sonbufs_numgt1_;   // Number of son buffer with more 1 son;

  IME_SIZE_T total_lma_node_num_;
 ...
 private:
  ...
  void set_son_offset(LmaNodeGE1 *node, IME_SIZE_T offset);

  // Update the offset of homophonies' ids for a node.
  void set_homo_id_buf_offset(LmaNodeGE1 *node, IME_SIZE_T offset);
  ...
  bool construct_subset(void* parent, LemmaEntry* lemma_arr,
                        IME_SIZE_T item_start, IME_SIZE_T item_end, IME_SIZE_T level);
  ...
  char16* read_valid_hanzis(const char *fn_validhzs, IME_SIZE_T *num);
  ...
  IME_SIZE_T read_raw_dict(const char* fn_raw, const char *fn_validhzs,
                       IME_SIZE_T max_item);

  // Try to find if a character is in hzs buffer.
  bool hz_in_hanzis_list(const char16 *hzs, IME_SIZE_T hzs_len, char16 hz);

  // Try to find if all characters in str are in hzs buffer.
  bool str_in_hanzis_list(const char16 *hzs, IME_SIZE_T hzs_len,
  ...
  bool alloc_resource(IME_SIZE_T lma_num);
