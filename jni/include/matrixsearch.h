...
static const IME_SIZE_T kMaxRowNum = kMaxSearchSteps;
...
class MatrixSearch {
 private:
  ...
  static const IME_SIZE_T kMaxLmaPsbItems = 1450;

  // How many rows for each step.
  static const IME_SIZE_T kMaxNodeARow = 5;

  // The maximum length of the sentence candidates counted in chinese
  // characters
  static const IME_SIZE_T kMaxSentenceLength = 16;

  // The size of the matrix node pool.
  static const IME_SIZE_T kMtrxNdPoolSize = 200;

  // The size of the DMI node pool.
  static const IME_SIZE_T kDmiPoolSize = 800;
  ...
  // candidate.
  IME_SIZE_T spl_id_num_;                // Number of splling ids
  ...
  bool reset_search(IME_SIZE_T ch_pos, bool clear_fixed_this_step,
                    bool clear_dmi_this_step, bool clear_mtrx_this_step);

  // Delete a part of the content in pys_.
  void del_in_pys(IME_SIZE_T start, IME_SIZE_T len);
  ...
  // the fixed lemmas.
  void merge_fixed_lmas(IME_SIZE_T del_spl_pos);
  ...
  IME_SIZE_T get_lpis(const uint16* splid_str, IME_SIZE_T splid_str_len,
  ...
  IME_SIZE_T extend_dmi(DictExtPara *dep, DictMatchInfo *dmi_s);

  // Extend dmi for the composing phrase.
  IME_SIZE_T extend_dmi_c(DictExtPara *dep, DictMatchInfo *dmi_s);
  ...
  // calling this function if necessary.
  // return 0 always.
  IME_SIZE_T extend_mtrx_nd(MatrixNode *mtrx_nd, LmaPsbItem lpi_items[],
                        IME_SIZE_T lpi_num, PoolPosType dmi_fr, IME_SIZE_T res_row);


  // Try to find a dmi node at step_to position, and the found dmi node should
  // match the given spelling id strings.
  PoolPosType match_dmi(IME_SIZE_T step_to, uint16 spl_ids[], uint16 spl_id_num);
  ...
  IME_SIZE_T inner_predict(const char16 fixed_scis_ids[], uint16 scis_num,
 ...
 public:
  ...
  void set_max_lens(IME_SIZE_T max_sps_len, IME_SIZE_T max_hzs_len);
  ...
  IME_SIZE_T search(const char *py, IME_SIZE_T py_len);
  ...
  IME_SIZE_T get_spl_start(const uint16 *&spl_start);

  // Get one candiate string. If full sentence candidate is available, it will
  // be the first one.
  char16* get_candidate(IME_SIZE_T cand_id, char16 *cand_str, IME_SIZE_T max_len);
