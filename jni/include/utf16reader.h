...
class Utf16Reader {
 private:
  FILE *fp_;
  char16 *buffer_;
  IME_SIZE_T buffer_total_len_;
  IME_SIZE_T buffer_next_pos_;

  // Always less than buffer_total_len_ - buffer_next_pos_
  IME_SIZE_T buffer_valid_len_;
 ...
 public:
  ...
  // future reading
  bool open(const char* filename, IME_SIZE_T buffer_len);
  char16* readline(char16* read_buf, IME_SIZE_T max_len);
  bool close();
};
}
