...
void SpellingTrie::free_son_trie(SpellingNode* node) {
  ...
  for (IME_SIZE_T pos = 0; pos < node->num_of_son; pos++) {
...
bool SpellingTrie::construct(const char* spelling_arr, IME_SIZE_T item_size,
                             IME_SIZE_T item_num, float score_amplifier,
...
bool SpellingTrie::build_ym_info() {
  ...
  IME_SIZE_T ym_item_size;  // '\0' is included
  IME_SIZE_T ym_num;
  const char* ym_buf;
...
SpellingNode* SpellingTrie::construct_spellings_subset(
    IME_SIZE_T item_start, IME_SIZE_T item_end, IME_SIZE_T level, SpellingNode* parent) {
  ...
  for (IME_SIZE_T i = item_start + 1; i < item_end; i++) {
    ...
    if (char_current != char_for_node) {
      ...
      if (spelling_last_start[level + 1] != '\0' || i - item_start_next > 1) {
        IME_SIZE_T real_start = item_start_next;
        if (spelling_last_start[level + 1] == '\0')
          real_start++;
...
bool SpellingTrie::save_spl_trie(FILE *fp) {
  ...
  if (fwrite(&spelling_size_, 4 /*sizeof(IME_SIZE_T)*/, 1, fp) != 1)
    return false;

  if (fwrite(&spelling_num_, 4 /*sizeof(IME_SIZE_T)*/, 1, fp) != 1)
    return false;
...
bool SpellingTrie::load_spl_trie(FILE *fp) {
  ...
  if (fread(&spelling_size_, 4 /*sizeof(IME_SIZE_T)*/, 1, fp) != 1)
    return false;

  if (fread(&spelling_num_, 4 /*sizeof(IME_SIZE_T)*/, 1, fp) != 1)
    return false;
...
IME_SIZE_T SpellingTrie::get_spelling_num() {
...
const char16* SpellingTrie::get_spelling_str16(uint16 splid) {
  ...
  if (splid >= kFullSplIdStart) {
    splid -= kFullSplIdStart;
    for (IME_SIZE_T pos = 0; pos < spelling_size_; pos++) {
...
IME_SIZE_T SpellingTrie::get_spelling_str16(uint16 splid, char16 *splstr16,
                                        IME_SIZE_T splstr16_len) {
  if (NULL == splstr16 || splstr16_len < kMaxPinyinSize + 1) return 0;

  if (splid >= kFullSplIdStart) {
    splid -= kFullSplIdStart;
    for (IME_SIZE_T pos = 0; pos <= kMaxPinyinSize; pos++) {
