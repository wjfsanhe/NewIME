...
IME_SIZE_T update_code_idx(double freqs[], IME_SIZE_T num, double code_book[],
                       CODEBOOK_TYPE *code_idx) {
  IME_SIZE_T changed = 0;
...
double recalculate_kernel(double freqs[], IME_SIZE_T num, double code_book[],
                          CODEBOOK_TYPE *code_idx) {
  double ret = 0;

  IME_SIZE_T *item_num =  new IME_SIZE_T[kCodeBookSize];
  assert(item_num);
  memset(item_num, 0, sizeof(IME_SIZE_T) * kCodeBookSize);
  ...
  for (IME_SIZE_T code = 0; code < kCodeBookSize; code++) {
...
void iterate_codes(double freqs[], IME_SIZE_T num, double code_book[],
                   CODEBOOK_TYPE *code_idx) {
  IME_SIZE_T iter_num = 0;
  double delta_last = 0;
  do {
    IME_SIZE_T changed = update_code_idx(freqs, num, code_book, code_idx);
...
bool NGram::save_ngram(FILE *fp) {
  ...
  if (fwrite(&idx_num_, sizeof(IME_SIZE_T), 1, fp) != 1)
    return false;
...
bool NGram::load_ngram(FILE *fp) {
  ...
  if (fread(&idx_num_, sizeof(IME_SIZE_T), 1, fp) != 1 )
    return false;
...
void NGram::set_total_freq_none_sys(IME_SIZE_T freq_none_sys) {
...
#ifdef ___BUILD_MODEL___
bool NGram::build_unigram(LemmaEntry *lemma_arr, IME_SIZE_T lemma_num,
  ...
  for (IME_SIZE_T pos = 0; pos < lemma_num; pos++) {
  ...
  for (IME_SIZE_T pos = 0; pos < idx_num_; pos++) {
  ...
  IME_SIZE_T freq_pos = 0;
  for (IME_SIZE_T code_pos = 0; code_pos < kCodeBookSize; code_pos++) {
    bool found = true;

    while (found) {
      found = false;
      double cand = freqs[freq_pos];
      for (IME_SIZE_T i = 0; i < code_pos; i++)
