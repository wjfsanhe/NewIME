...
IME_SIZE_T get_odd_next(IME_SIZE_T value) {
  IME_SIZE_T v_next = value;
  while (true) {
    IME_SIZE_T v_next_sqrt = (IME_SIZE_T)sqrt(v_next);

    bool is_odd = true;
    for (IME_SIZE_T v_dv = 2; v_dv < v_next_sqrt + 1; v_dv++) {
      if (v_next % v_dv == 0) {
        is_odd = false;
        break;
      }
...
IME_SIZE_T SpellingTable::get_hash_pos(const char* spelling_str) {
  IME_SIZE_T hash_pos = 0;
  for (IME_SIZE_T pos = 0; pos < spelling_size_; pos++) {
    if ('\0' == spelling_str[pos])
      break;
    hash_pos += (IME_SIZE_T)spelling_str[pos];
  }
...
IME_SIZE_T SpellingTable::hash_pos_next(IME_SIZE_T hash_pos) {
  hash_pos += 123;
  hash_pos = hash_pos % spelling_max_num_;
  return hash_pos;
}
...
bool SpellingTable::init_table(IME_SIZE_T pure_spl_size, IME_SIZE_T spl_max_num,
                               bool need_score) {
...
bool SpellingTable::put_spelling(const char* spelling_str, double freq) {
  ...
  for (IME_SIZE_T pos = 0; pos < kNotSupportNum; pos++) {
  ...
  IME_SIZE_T hash_pos = get_hash_pos(spelling_str);
  ...
  IME_SIZE_T hash_pos_ori = hash_pos;
...
const char* SpellingTable::arrange(IME_SIZE_T *item_size, IME_SIZE_T *spl_num) {
  if (NULL == raw_spellings_ || NULL == spelling_buf_ ||
  ...
  for (IME_SIZE_T pos = 0; pos < spelling_num_; pos++) {
