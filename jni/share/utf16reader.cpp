...
bool Utf16Reader::open(const char* filename, IME_SIZE_T buffer_len) {
  if (filename == NULL)
    return false;
...
char16* Utf16Reader::readline(char16* read_buf, IME_SIZE_T max_len) {
  if (NULL == fp_ || NULL == read_buf || 0 == max_len)
    return NULL;

  IME_SIZE_T ret_len = 0;
  ...
  do {
    ...
    for (IME_SIZE_T i = 0; i < buffer_valid_len_; i++) {
      if (i == max_len - 1 ||
