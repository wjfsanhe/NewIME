...
IME_SIZE_T UserDict::number_of_lemmas() {
...
IME_SIZE_T UserDict::get_lpis(const uint16 *splid_str, uint16 splid_str_len,
                          LmaPsbItem *lpi_items, IME_SIZE_T lpi_max) {
  return _get_lpis(splid_str, splid_str_len, lpi_items, lpi_max, NULL);
}

IME_SIZE_T UserDict::_get_lpis(const uint16 *splid_str,
                           uint16 splid_str_len, LmaPsbItem *lpi_items,
                           IME_SIZE_T lpi_max, bool * need_extend) {
  bool tmp_extend;
  if (!need_extend)
  ...
  *need_extend = false;
  ...
  IME_SIZE_T lpi_current = 0;

  bool fuzzy_break = false;
  bool prefix_break = false;
  while ((IME_SIZE_T)middle < max_off && !fuzzy_break && !prefix_break) {
...
IME_SIZE_T UserDict::predict(const char16 last_hzs[], uint16 hzs_len,
...
bool UserDict::reset(const char *file) {
  ...
  IME_SIZE_T wred = fwrite(&version, 1, 4, fp);
  UserDictInfo info;
...
void UserDict::write_back_lemma(int fd) {
  ...
  IME_SIZE_T need_write = kUserDictPreAlloc *
...
void UserDict::defragment(void) {
  ...
  IME_SIZE_T first_freed = 0;
  IME_SIZE_T first_inuse = 0;
  ...
  IME_SIZE_T real_size = total_size - lemma_size_left_;
  while (dst < real_size) {
 ...
 repeat:
    ...
    for (IME_SIZE_T j = 0; j < dict_info_.lemma_count; j++) {
...
#ifdef ___SYNC_ENABLED___
    for (IME_SIZE_T j = 0; j < dict_info_.sync_count; j++) {
...
LemmaIdType UserDict::_put_lemma(char16 lemma_str[], uint16 splids[],
  ...
  if (off != -1) {
    ...
    if (lemma_count_left_ == 0 ||
        lemma_size_left_ < (IME_SIZE_T)(2 + (lemma_len << 2))) {
...
IME_SIZE_T UserDict::get_total_lemma_count() {
  return dict_info_.total_nfreq;
}

void UserDict::set_total_lemma_count_of_others(IME_SIZE_T count) {
...
LemmaIdType UserDict::append_a_lemma(char16 lemma_str[], uint16 splids[],
  ...
  IME_SIZE_T offset = dict_info_.lemma_size;
  if (offset > kUserDictOffsetMask)
    return 0;

  lemmas_[offset] = 0;
  lemmas_[offset + 1] = (uint8)lemma_len;
  for (IME_SIZE_T i = 0; i < lemma_len; i++) {
