...
extern "C" {
  ...
  static const IME_SIZE_T kMaxPredictNum = 500;
  ...
  void im_set_max_lens(IME_SIZE_T max_sps_len, IME_SIZE_T max_hzs_len) {
    if (NULL != matrix_search) {
  ...
  // To be updated.
  IME_SIZE_T im_search(const char* pybuf, IME_SIZE_T pylen) {
    if (NULL == matrix_search)
      return 0;
  ...
  IME_SIZE_T im_delsearch(IME_SIZE_T pos, bool is_pos_in_splid,
  ...
  // To be removed
  IME_SIZE_T im_add_letter(char ch) {
    return 0;
  }

  const char* im_get_sps_str(IME_SIZE_T *decoded_len) {
    if (NULL == matrix_search)
      return NULL;
  ...
  char16* im_get_candidate(IME_SIZE_T cand_id, char16* cand_str,
                        IME_SIZE_T max_len) {
    if (NULL == matrix_search)
      return NULL;
  ...
  IME_SIZE_T im_get_spl_start_pos(const uint16 *&spl_start) {
    if (NULL == matrix_search)
      return 0;
  ...
  IME_SIZE_T im_choose(IME_SIZE_T choice_id) {
    if (NULL == matrix_search)
      return 0;
  ...
  IME_SIZE_T im_cancel_last_choice() {
    if (NULL == matrix_search)
      return 0;
  ...
  IME_SIZE_T im_get_predicts(const char16 *his_buf,
                         char16 (*&pre_buf)[kMaxPredictSize + 1]) {
    if (NULL == his_buf)
      return 0;

    IME_SIZE_T fixed_len = utf16_strlen(his_buf);
