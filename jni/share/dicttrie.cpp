...
inline IME_SIZE_T DictTrie::get_son_offset(const LmaNodeGE1 *node) {
  return ((IME_SIZE_T)node->son_1st_off_l + ((IME_SIZE_T)node->son_1st_off_h << 16));
}

inline IME_SIZE_T DictTrie::get_homo_idx_buf_offset(const LmaNodeGE1 *node) {
  return ((IME_SIZE_T)node->homo_idx_buf_off_l +
          ((IME_SIZE_T)node->homo_idx_buf_off_h << 16));
}

inline LemmaIdType DictTrie::get_lemma_id(IME_SIZE_T id_offset) {
  LemmaIdType id = 0;
...
bool DictTrie::save_dict(FILE *fp) {
  ...
  if (fwrite(&lma_node_num_le0_, sizeof(IME_SIZE_T), 1, fp) != 1)
    return false;

  if (fwrite(&lma_node_num_ge1_, sizeof(IME_SIZE_T), 1, fp) != 1)
    return false;

  if (fwrite(&lma_idx_buf_len_, sizeof(IME_SIZE_T), 1, fp) != 1)
    return false;

  if (fwrite(&top_lmas_num_, sizeof(IME_SIZE_T), 1, fp) != 1)
    return false;
...
bool DictTrie::load_dict(FILE *fp) {
  ...
  if (fread(&lma_node_num_le0_, sizeof(IME_SIZE_T), 1, fp) != 1)
    return false;

  if (fread(&lma_node_num_ge1_, sizeof(IME_SIZE_T), 1, fp) != 1)
    return false;

  if (fread(&lma_idx_buf_len_, sizeof(IME_SIZE_T), 1, fp) != 1)
    return false;

  if (fread(&top_lmas_num_, sizeof(IME_SIZE_T), 1, fp) != 1 ||
  ...
  IME_SIZE_T buf_size = SpellingTrie::get_instance().get_spelling_num() + 1;
  ...
  IME_SIZE_T last_pos = 0;
  for (IME_SIZE_T i = 1; i < lma_node_num_le0_; i++) {
  ...
  for (uint16 splid = last_splid + 1;
    ...
    assert(static_cast<IME_SIZE_T>(splid - kFullSplIdStart) < buf_size);
...
IME_SIZE_T DictTrie::fill_lpi_buffer(LmaPsbItem lpi_items[], IME_SIZE_T lpi_max,
                                 LmaNodeLE0 *node) {
  IME_SIZE_T lpi_num = 0;
  NGram& ngram = NGram::get_instance();
  for (IME_SIZE_T homo = 0; homo < (IME_SIZE_T)node->num_of_homo; homo++) {
...
IME_SIZE_T DictTrie::fill_lpi_buffer(LmaPsbItem lpi_items[], IME_SIZE_T lpi_max,
                                 IME_SIZE_T homo_buf_off, LmaNodeGE1 *node,
                                 uint16 lma_len) {
  IME_SIZE_T lpi_num = 0;
  NGram& ngram = NGram::get_instance();
  for (IME_SIZE_T homo = 0; homo < (IME_SIZE_T)node->num_of_homo; homo++) {
...
MileStoneHandle DictTrie::extend_dict(MileStoneHandle from_handle,
                                      ...
                                      LmaPsbItem *lpi_items, IME_SIZE_T lpi_max,
                                      IME_SIZE_T *lpi_num) {
  if (NULL == dep)
    return 0;
...
MileStoneHandle DictTrie::extend_dict0(MileStoneHandle from_handle,
                                       ...
                                       IME_SIZE_T lpi_max, IME_SIZE_T *lpi_num) {
  ...
  *lpi_num = 0;
  ...
  IME_SIZE_T son_start = splid_le0_index_[id_start - kFullSplIdStart];
  IME_SIZE_T son_end = splid_le0_index_[id_start + id_num - kFullSplIdStart];
  for (IME_SIZE_T son_pos = son_start; son_pos < son_end; son_pos++) {
...
MileStoneHandle DictTrie::extend_dict1(MileStoneHandle from_handle,
  ...
  for (uint16 h_pos = 0; h_pos < mile_stone->mark_num; h_pos++) {
    ...
    for (uint16 ext_pos = 0; ext_pos < ext_num; ext_pos++) {
      ...
      IME_SIZE_T found_start = 0;
      IME_SIZE_T found_num = 0;
      for (IME_SIZE_T son_pos = 0; son_pos < (IME_SIZE_T)node->num_of_son; son_pos++) {
        assert(node->son_1st_off <= lma_node_num_ge1_);
        LmaNodeGE1 *son = nodes_ge1_ + node->son_1st_off + son_pos;
        if (son->spl_idx >= id_start
            && son->spl_idx < id_start + id_num) {
          if (*lpi_num < lpi_max) {
            IME_SIZE_T homo_buf_off = get_homo_idx_buf_offset(son);
            *lpi_num += fill_lpi_buffer(lpi_items + (*lpi_num),
...
bool DictTrie::try_extend(const uint16 *splids, uint16 splid_num,
  ...
  if (1 == splid_num) {
    LmaNodeLE0* node_le0 = reinterpret_cast<LmaNodeLE0*>(node);
    IME_SIZE_T num_of_homo = (IME_SIZE_T)node_le0->num_of_homo;
    for (IME_SIZE_T homo_pos = 0; homo_pos < num_of_homo; homo_pos++) {
    ...
    LmaNodeGE1* node_ge1 = reinterpret_cast<LmaNodeGE1*>(node);
    IME_SIZE_T num_of_homo = (IME_SIZE_T)node_ge1->num_of_homo;
    for (IME_SIZE_T homo_pos = 0; homo_pos < num_of_homo; homo_pos++) {
      IME_SIZE_T node_homo_off = get_homo_idx_buf_offset(node_ge1);
...
IME_SIZE_T DictTrie::get_lpis(const uint16* splid_str, uint16 splid_str_len,
...
#define MAX_EXTENDBUF_LEN 200
  ...
  IME_SIZE_T* node_buf1[MAX_EXTENDBUF_LEN];  // use IME_SIZE_T for data alignment
  IME_SIZE_T* node_buf2[MAX_EXTENDBUF_LEN];
  ...
  IME_SIZE_T node_fr_num = 1;
  IME_SIZE_T node_to_num = 0;
  node_fr_le0[0] = root_;
  if (NULL == node_fr_le0[0])
    return 0;

  IME_SIZE_T spl_pos = 0;
  ...
  while (spl_pos < splid_str_len) {
    ...
    if (0 == spl_pos) {  // From LmaNodeLE0 (root) to LmaNodeLE0 nodes
      for (IME_SIZE_T node_fr_pos = 0; node_fr_pos < node_fr_num; node_fr_pos++) {
      ...
      for (IME_SIZE_T node_fr_pos = 0; node_fr_pos < node_fr_num; node_fr_pos++) {
        LmaNodeLE0 *node = node_fr_le0[node_fr_pos];
        for (IME_SIZE_T son_pos = 0; son_pos < (IME_SIZE_T)node->num_of_son;
             son_pos++) {
          assert(node->son_1st_off <= lma_node_num_ge1_);
  ...
  for (IME_SIZE_T node_pos = 0; node_pos < node_to_num; node_pos++) {
    IME_SIZE_T num_of_homo = 0;
    if (spl_pos <= 1) {  // Get from LmaNodeLE0 nodes
      LmaNodeLE0* node_le0 = node_to_le0[node_pos];
      num_of_homo = (IME_SIZE_T)node_le0->num_of_homo;
      for (IME_SIZE_T homo_pos = 0; homo_pos < num_of_homo; homo_pos++) {
        IME_SIZE_T ch_pos = lma_num + homo_pos;
        lma_buf[ch_pos].id =
      ...
      num_of_homo = (IME_SIZE_T)node_ge1->num_of_homo;
      for (IME_SIZE_T homo_pos = 0; homo_pos < num_of_homo; homo_pos++) {
        IME_SIZE_T ch_pos = lma_num + homo_pos;
        IME_SIZE_T node_homo_off = get_homo_idx_buf_offset(node_ge1);
...
void DictTrie::set_total_lemma_count_of_others(IME_SIZE_T count) {
  NGram& ngram = NGram::get_instance();
...
IME_SIZE_T DictTrie::predict_top_lmas(IME_SIZE_T his_len, NPredictItem *npre_items,
                                  IME_SIZE_T npre_max, IME_SIZE_T b4_used) {
  NGram &ngram = NGram::get_instance();

  IME_SIZE_T item_num = 0;
  IME_SIZE_T top_lmas_id_offset = lma_idx_buf_len_ / kLemmaIdSize - top_lmas_num_;
  IME_SIZE_T top_lmas_pos = 0;
...
IME_SIZE_T DictTrie::predict(const char16 *last_hzs, uint16 hzs_len,
                         NPredictItem *npre_items, IME_SIZE_T npre_max,
                         IME_SIZE_T b4_used) {
