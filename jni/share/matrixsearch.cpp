...
bool MatrixSearch::alloc_resource() {
  ...
  IME_SIZE_T mtrx_nd_size = sizeof(MatrixNode) * kMtrxNdPoolSize;
  mtrx_nd_size = align_to_IME_SIZE_T(mtrx_nd_size) / sizeof(IME_SIZE_T);
  IME_SIZE_T dmi_size = sizeof(DictMatchInfo) * kDmiPoolSize;
  dmi_size = align_to_IME_SIZE_T(dmi_size) / sizeof(IME_SIZE_T);
  IME_SIZE_T matrix_size = sizeof(MatrixRow) * kMaxRowNum;
  matrix_size = align_to_IME_SIZE_T(matrix_size) / sizeof(IME_SIZE_T);
  IME_SIZE_T dep_size = sizeof(DictExtPara);
  dep_size = align_to_IME_SIZE_T(dep_size) / sizeof(IME_SIZE_T);

  // share_buf's size is determined by the buffers for search.
  share_buf_ = new IME_SIZE_T[mtrx_nd_size + dmi_size + matrix_size + dep_size];
  ...
  npre_items_len_ = (mtrx_nd_size + dmi_size + matrix_size + dep_size) *
      sizeof(IME_SIZE_T) / sizeof(NPredictItem);
  return true;
}
...
void MatrixSearch::set_max_lens(IME_SIZE_T max_sps_len, IME_SIZE_T max_hzs_len) {
  if (0 != max_sps_len)
...
bool MatrixSearch::reset_search(IME_SIZE_T ch_pos, bool clear_fixed_this_step,
  ...
  if (0 == ch_pos) {
    ...
    if (fixed_hzs_ > 0 &&
      ...
      IME_SIZE_T fixed_ch_pos = ch_pos;
...
void MatrixSearch::del_in_pys(IME_SIZE_T start, IME_SIZE_T len) {
...
IME_SIZE_T MatrixSearch::search(const char *py, IME_SIZE_T py_len) {
  if (!inited_ || NULL == py)
    return 0;
...
IME_SIZE_T MatrixSearch::delsearch(IME_SIZE_T pos, bool is_pos_in_splid,
                               bool clear_fixed_this_step) {
  if (!inited_)
    return 0;

  IME_SIZE_T reset_pos = pos;
  ...
  IME_SIZE_T c_py_len = 0;  // The length of composing phrase's Pinyin
  IME_SIZE_T del_py_len = 1;
  if (!is_pos_in_splid) {
...
IME_SIZE_T MatrixSearch::get_candidate_num() {
...
char16* MatrixSearch::get_candidate(IME_SIZE_T cand_id, char16 *cand_str,
                                    IME_SIZE_T max_len) {
...
void MatrixSearch::update_dict_freq() {
  if (NULL != user_dict_) {
    ...
    IME_SIZE_T total_freq = user_dict_->get_total_lemma_count();
...
bool MatrixSearch::try_add_cand0_to_userdict() {
  IME_SIZE_T new_cand_num = get_candidate_num();
...
//                by 1.
IME_SIZE_T MatrixSearch::choose(IME_SIZE_T cand_id) {
  if (!inited_ || 0 == pys_decoded_len_)
    return 0;
  ...
  if (0 == cand_id) {
    ...
    matrix_[spl_start_[fixed_hzs_]].mtrx_nd_fixed = mtrx_nd_pool_ +
        matrix_[spl_start_[fixed_hzs_]].mtrx_nd_pos;
    for (IME_SIZE_T pos = fixed_lmas_; pos < lma_id_num_; pos++) {
  ...
  IME_SIZE_T cand_len = lpi_items_[cand_id].lma_len;
  ...
  IME_SIZE_T step_fr = spl_start_[fixed_hzs_];
  IME_SIZE_T step_to = spl_start_[fixed_hzs_ + cand_len];

  // 3.2 Save the length of the original string.
  IME_SIZE_T pys_decoded_len = pys_decoded_len_;
...
IME_SIZE_T MatrixSearch::cancel_last_choice() {
  if (!inited_ || 0 == pys_decoded_len_)
    return 0;

  IME_SIZE_T step_start = 0;
  if (fixed_hzs_ > 0) {
    IME_SIZE_T step_end = spl_start_[fixed_hzs_];
...
IME_SIZE_T MatrixSearch::get_fixedlen() {
  if (!inited_ || 0 == pys_decoded_len_)
    return 0;
  return fixed_hzs_;
}
...
void MatrixSearch::prepare_candidates() {
  ...
  lpi_total_ = 0;
  IME_SIZE_T lpi_num_full_match = 0;  // Number of items which are fully-matched.
  while (lma_size > 0) {
    IME_SIZE_T lma_num;
    lma_num = get_lpis(spl_id_ + fixed_hzs_, lma_size,
                       lpi_items_ + lpi_total_,
                       IME_SIZE_T(kMaxLmaPsbItems - lpi_total_),
  ...
  if (kPrintDebug0) {
    ...
    for (IME_SIZE_T a = 0; a < lpi_total_; a++) {
...
const char* MatrixSearch::get_pystr(IME_SIZE_T *decoded_len) {
...
void MatrixSearch::get_spl_start_id() {
  ...
  for (IME_SIZE_T pos = fixed_lmas_;
  ...
  for (IME_SIZE_T pos = fixed_lmas_ + 1; pos <= lma_id_num_; pos++) {
    if (pos < lma_id_num_)
  ...
  fixed_hzs_ = 0;
  for (IME_SIZE_T pos = spl_id_num_; pos > 0; pos--) {
...
IME_SIZE_T MatrixSearch::get_spl_start(const uint16 *&spl_start) {
  get_spl_start_id();
  spl_start = spl_start_;
  return spl_id_num_;
}

IME_SIZE_T MatrixSearch::extend_dmi(DictExtPara *dep, DictMatchInfo *dmi_s) {
  ...
  if (NULL != user_dict_ && (from_h[1] > 0 || NULL == dmi_s)) {
    ...
    if (handles[1] > 0) {
      if (kPrintDebug0) {
        for (IME_SIZE_T t = 0; t < lpi_num; t++) {
...
IME_SIZE_T MatrixSearch::extend_dmi_c(DictExtPara *dep, DictMatchInfo *dmi_s) {
  lpi_total_ = 0;
...
IME_SIZE_T MatrixSearch::extend_mtrx_nd(MatrixNode *mtrx_nd, LmaPsbItem lpi_items[],
                                    IME_SIZE_T lpi_num, PoolPosType dmi_fr,
                                    IME_SIZE_T res_row) {
  assert(NULL != mtrx_nd);
...
PoolPosType MatrixSearch::match_dmi(IME_SIZE_T step_to, uint16 spl_ids[],
                                    uint16 spl_id_num) {
...
char16* MatrixSearch::get_candidate0(char16 *cand_str, IME_SIZE_T max_len,
                                     uint16 *retstr_len,
                                     bool only_unfixed) {
...
IME_SIZE_T MatrixSearch::get_lpis(const uint16* splid_str, IME_SIZE_T splid_str_len,
                              LmaPsbItem* lma_buf, IME_SIZE_T max_lma_buf,
                              const char16 *pfullsent, bool sort_by_psb) {
  if (splid_str_len > kMaxLemmaSize)
    return 0;

  IME_SIZE_T num1 = dict_trie_->get_lpis(splid_str, splid_str_len,
                                     lma_buf, max_lma_buf);
  IME_SIZE_T num2 = 0;
  if (NULL != user_dict_) {
  ...
  if (splid_str_len > 1) {
    ...
    IME_SIZE_T lpsi_num = (max_lma_buf - num) * sizeof(LmaPsbItem) /
        sizeof(LmaPsbStrItem);
    assert(lpsi_num > num);
    if (num > lpsi_num) num = lpsi_num;
    lpsi_num = num;

    for (IME_SIZE_T pos = 0; pos < lpsi_num; pos++) {
    ...
    for (IME_SIZE_T pos = 0; pos < num; pos++) {
      char16 hanzis[2];
...
IME_SIZE_T MatrixSearch::inner_predict(const char16 *fixed_buf, uint16 fixed_len,
                                   char16 predict_buf[][kMaxPredictSize + 1],
                                   IME_SIZE_T buf_len) {
  IME_SIZE_T res_total = 0;
  ...
  for (IME_SIZE_T len = fixed_len; len >0; len--) {
    // How many blank items are available
    IME_SIZE_T this_max = npre_items_len_ - res_total;
    IME_SIZE_T res_this;
    ...
    if (fixed_len > 1 && 1 == len && 0 == res_total) {
      ...
      for (IME_SIZE_T nlen = 2; nlen <= fixed_len; nlen++) {
  ...
  if (kPrintDebug2) {
    ...
    for (IME_SIZE_T i = 0; i < res_total; i++) {
      printf("---");
      for (IME_SIZE_T j = 0; j < kMaxPredictSize; j++) {
...
IME_SIZE_T MatrixSearch::get_predicts(const char16 fixed_buf[],
                                  char16 predict_buf[][kMaxPredictSize + 1],
                                  IME_SIZE_T buf_len) {
  IME_SIZE_T fixed_len = utf16_strlen(fixed_buf);
