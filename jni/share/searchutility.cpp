...
int cmp_lpi_with_unified_psb(const void *p1, const void *p2) {
  ...
  // precision.
  IME_SIZE_T up1 = item1->psb * (item2->lma_len);
  IME_SIZE_T up2 = item2->psb * (item1->lma_len);
  if (up1 < up2) {
    return -1;
  }
  if (up1 > up2) {
...
IME_SIZE_T remove_duplicate_npre(NPredictItem *npre_items, IME_SIZE_T npre_num) {
  if (NULL == npre_items || 0 == npre_num)
    return 0;

  myqsort(npre_items, npre_num, sizeof(NPredictItem), cmp_npre_by_hanzi_score);

  IME_SIZE_T remain_num = 1;  // The first one is reserved.
  for (IME_SIZE_T pos = 1; pos < npre_num; pos++) {
...
IME_SIZE_T align_to_IME_SIZE_T(IME_SIZE_T size) {
  IME_SIZE_T s = sizeof(IME_SIZE_T);
  return (size + s -1) / s * s;
}
