...
static const IME_SIZE_T kReadBufLen = 512;
static const IME_SIZE_T kSplTableHashLen = 2000;
...
int cmp_lemma_entry_hzs(const void* p1, const void* p2) {
  IME_SIZE_T size1 = utf16_strlen(((const LemmaEntry*)p1)->hanzi_str);
  IME_SIZE_T size2 = utf16_strlen(((const LemmaEntry*)p2)->hanzi_str);
  if (size1 < size2)
    return -1;
  else if (size1 > size2)
    return 1;
...
bool DictBuilder::alloc_resource(IME_SIZE_T lma_num) {
  if (0 == lma_num)
    return false;
...
char16* DictBuilder::read_valid_hanzis(const char *fn_validhzs, IME_SIZE_T *num) {
...
bool DictBuilder::hz_in_hanzis_list(const char16 *hzs, IME_SIZE_T hzs_len,
                                    char16 hz) {
  if (NULL == hzs)
    return false;
...
bool DictBuilder::str_in_hanzis_list(const char16 *hzs, IME_SIZE_T hzs_len,
                                     const char16 *str, IME_SIZE_T str_len) {
  if (NULL == hzs || NULL == str)
    return false;

  for (IME_SIZE_T pos = 0; pos < str_len; pos++) {
...
void DictBuilder::get_top_lemmas() {
  ...
  for (IME_SIZE_T pos = 0; pos < lemma_num_; pos++) {
    if (0 == top_lmas_num_) {
  ...
  if (kPrintDebug0) {
    printf("\n------Top Lemmas------------------\n");
    for (IME_SIZE_T pos = 0; pos < top_lmas_num_; pos++) {
...
IME_SIZE_T DictBuilder::read_raw_dict(const char* fn_raw,
  ...
  for (IME_SIZE_T i = 0; i < max_item; i++) {
    ...
    IME_SIZE_T lemma_size = utf16_strlen(token);
    ...
    for (IME_SIZE_T hz_pos = 0; hz_pos < (IME_SIZE_T)lemma_arr_[i].hz_str_len;
         hz_pos++) {
      // Get a Pinyin
      token = utf16_strtok(to_tokenize, &token_size, &to_tokenize);
      if (NULL == token) {
...
bool DictBuilder::build_dict(const char *fn_raw,
  ...
  for (IME_SIZE_T i = 0; i < lemma_num_; i++) {
    for (IME_SIZE_T hz_pos = 0; hz_pos < (IME_SIZE_T)lemma_arr_[i].hz_str_len;
         hz_pos++) {
      uint16 spl_idxs[2];
  ...
  IME_SIZE_T lma_idx_num = homo_idx_num_eq1_ + homo_idx_num_gt1_ + top_lmas_num_;
  ...
  for (IME_SIZE_T pos = 0; pos < homo_idx_num_eq1_ + homo_idx_num_gt1_; pos++) {
    id_to_charbuf(dict_trie->lma_idx_buf_ + pos * kLemmaIdSize,
                  homo_idx_buf_[pos]);
  }

  for (IME_SIZE_T pos = homo_idx_num_eq1_ + homo_idx_num_gt1_;
       pos < lma_idx_num; pos++) {
    LemmaIdType idx =
...
void DictBuilder::id_to_charbuf(unsigned char *buf, LemmaIdType id) {
  if (NULL == buf) return;
  for (IME_SIZE_T pos = 0; pos < kLemmaIdSize; pos++) {
    (buf)[pos] = (unsigned char)(id >> (pos * 8));
  }
}

void DictBuilder::set_son_offset(LmaNodeGE1 *node, IME_SIZE_T offset) {
  node->son_1st_off_l = static_cast<uint16>(offset);
  node->son_1st_off_h = static_cast<unsigned char>(offset >> 16);
}

void DictBuilder:: set_homo_id_buf_offset(LmaNodeGE1 *node, IME_SIZE_T offset) {
...
LemmaIdType DictBuilder::sort_lemmas_by_hz() {
  ...
  for (IME_SIZE_T i = 1; i < lemma_num_; i++) {
...
IME_SIZE_T DictBuilder::build_scis() {
  ...
  for (IME_SIZE_T pos = 0; pos < lemma_num_; pos++) {
    IME_SIZE_T hz_num = lemma_arr_[pos].hz_str_len;
    for (IME_SIZE_T hzpos = 0; hzpos < hz_num; hzpos++) {
  ...
  IME_SIZE_T unique_scis_num = 1;
  for (IME_SIZE_T pos = 1; pos < scis_num_; pos++) {
...
bool DictBuilder::construct_subset(void* parent, LemmaEntry* lemma_arr,
                                   IME_SIZE_T item_start, IME_SIZE_T item_end,
                                   IME_SIZE_T level) {
  if (level >= kMaxLemmaSize || item_end <= item_start)
    return false;

  // 1. Scan for how many sons
  IME_SIZE_T parent_son_num = 0;
  ...
  for (IME_SIZE_T i = item_start + 1; i< item_end; i++) {
  ...
  for (IME_SIZE_T i = item_start + 1; i < item_end; i++) {
    ...
    if (spl_idx_current == spl_idx_node) {
      ...
      if (homo_num > 0) {
        ...
        for (IME_SIZE_T homo_pos = 0; homo_pos < homo_num; homo_pos++) {
...
void DictBuilder::stat_init() {
  memset(max_sonbuf_len_, 0, sizeof(IME_SIZE_T) * kMaxLemmaSize);
  memset(max_homobuf_len_, 0, sizeof(IME_SIZE_T) * kMaxLemmaSize);
  memset(total_son_num_, 0, sizeof(IME_SIZE_T) * kMaxLemmaSize);
  memset(total_node_hasson_, 0, sizeof(IME_SIZE_T) * kMaxLemmaSize);
  memset(total_sonbuf_num_, 0, sizeof(IME_SIZE_T) * kMaxLemmaSize);
  memset(total_sonbuf_allnoson_, 0, sizeof(IME_SIZE_T) * kMaxLemmaSize);
  memset(total_node_in_sonbuf_allnoson_, 0, sizeof(IME_SIZE_T) * kMaxLemmaSize);
  memset(total_homo_num_, 0, sizeof(IME_SIZE_T) * kMaxLemmaSize);
...
void DictBuilder::stat_print() {
  ...
  for (IME_SIZE_T i = 0; i < kMaxLemmaSize; i++)
