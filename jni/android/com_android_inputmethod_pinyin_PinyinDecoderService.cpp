...
JNIEXPORT void JNICALL nativeImSetMaxLens(JNIEnv* env, jclass jclazz,
                                          jint max_sps_len,
                                          jint max_hzs_len) {
  im_set_max_lens(static_cast<IME_SIZE_T>(max_sps_len),
                  static_cast<IME_SIZE_T>(max_hzs_len));
  return;
}
...
JNIEXPORT jint JNICALL nativeImGetPredictsNum(JNIEnv *env, jclass clazz,
                                              jstring fixed_str) {
  jboolean list=0; 
  char16 *fixed_ptr = (char16*)(*env).GetStringChars(fixed_str, &list);
  IME_SIZE_T fixed_len = (IME_SIZE_T)(*env).GetStringLength(fixed_str);
...
JNIEXPORT jstring JNICALL nativeImGetPredictItem(JNIEnv *env, jclass clazz,
  ...
  if (predict_no < 0 || (IME_SIZE_T)predict_no >= predict_len) {
...
JNIEXPORT jint JNICALL nativeSyncPutLemmas(JNIEnv *env, jclass clazz,
  ...
  int len = (IME_SIZE_T)(*env).GetStringLength(tomerge);
